...
//   <p class="footer">these ${petsData.length} pets were added recently. check back soon for updates</P>`
  ...
  // array of peole name and positiom

  const titles = [
      {
          name : "Joyce",
          job: 'Property Managment',
          phone: "612-821-2111",
          company: "MPHA"
  ...
  // templete function for arrayMap
  const detail = (person) => {
      return `
      <div class='animal person'>
      <img class="pet-photo" src="${person.photo ? person.photo : ""}">  
      <h2 class='pet-name person '> ${person.name}</h2>
      <h3 class="job:>Company: ${person.company}</h3>
      <h3 class ="job">Title: ${person.job}</h3>
      <h3 class="job">Phone: ${person.phone}</h3>
...
${titles.map(detail).join('')}
  <footer class="footer">These ${titles.length} were added recently</footer>
  `